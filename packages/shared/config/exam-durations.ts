...
export const EXAM_DURATIONS = Object.freeze(
  Object.fromEntries(EXAM_DURATION_RULES.map((rule) => [rule.label, formatDuration(rule.minutes)])),
) as Readonly<Record<string, string>>

export type ExamDurationKey = keyof typeof EXAM_DURATIONS
...
export function getExamDuration(procedimento: string): string {
  const minutes = getExamDurationMinutes(procedimento)
  return minutes == null ? 'Tempo de exame nao definido' : formatDuration(minutes)
}
